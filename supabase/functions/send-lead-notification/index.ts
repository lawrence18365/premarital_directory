...
serve(async (req) => {
  ...
  try {
    ...
    const leadsUrl = `${siteUrl}/professional/leads`
    const dashboardUrl = `${siteUrl}/professional/dashboard`
    ...
    const subject = `New inquiry from ${coupleData.name} - ${inquiryReference}`
    
    const htmlContent = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background: #1f2937; padding: 24px; border-radius: 8px 8px 0 0; text-align: center;">
          <h1 style="color: white; margin: 0; font-size: 22px;">New inquiry from your listing</h1>
        </div>
        ...
        <div style="background: white; padding: 30px; border: 1px solid #e0e0e0; border-top: none; border-radius: 0 0 8px 8px;">
          ...
          <div style="margin: 30px 0; text-align: center;">
            <a href="${leadsUrl}" style="display: inline-block; background: #111827; color: white; padding: 12px 22px; text-decoration: none; border-radius: 6px; font-weight: 600; margin-right: 8px;">View inquiry</a>
            <a href="${dashboardUrl}" style="display: inline-block; background: white; color: #111827; border: 1px solid #d1d5db; padding: 12px 22px; text-decoration: none; border-radius: 6px; font-weight: 600;">Open dashboard</a>
          </div>
